"""Command-line interface.
    ...
    exportube import takeout.zip        # Google Takeout archive or extracted dir
    exportube import youtube            # browser-session watch history
    exportube scan                      # fetch YouTube/yt-dlp metadata + detect music
    exportube identify                  # MusicBrainz enrichment + confidence scoring
    exportube export output.csv         # write youtube_music_history.csv (+ canonical)
    exportube stats                     # progress / confidence distribution
    exportube serve                     # launch the local review web UI
...
from exportube.history_import.youtube_provider import YouTubeOAuthClient, YouTubeSessionProvider
...
from exportube.youtube_metadata.youtube_api_provider import YouTubeAPIProvider
...
def cli(ctx, config_path):
    """exportube: turn YouTube watch history into an enriched music catalog CSV."""
...
def identify(ctx, limit):
    """Run MusicBrainz (+ Discogs, if EXPORTUBE_DISCOGS_TOKEN is set)
    ...
    if discogs_token:
        ...
        click.echo("Discogs enrichment enabled (EXPORTUBE_DISCOGS_TOKEN set).")
...
def serve(ctx, host, port):
    ...
    cfg = ctx.obj["config"]
    from exportube.web.app import create_app
