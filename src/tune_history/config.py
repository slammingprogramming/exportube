...
Env var override format: TUNE_HISTORY_<PATH> where PATH is the dotted config
path with "__" separating nested keys, e.g.
TUNE_HISTORY_MATCHING__DURATION__STRONG_TOLERANCE_SECONDS=15
...
class Config:
    ...
    def db_path(self) -> Path:
        return Path(self.env("DB_PATH", self.get("storage.db_path", "./data/tune_history.sqlite3")))
    ...
    def musicbrainz_user_agent(self) -> tuple[str, str, str]:
        app = self.env("MUSICBRAINZ_APP", "tune-history")
