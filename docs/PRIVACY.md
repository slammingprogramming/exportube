...
tune-history is designed to run entirely on your machine. This document is
...
- Your raw watch history (imported Takeout files or the browser-session
  ...
  (`data/tune_history.sqlite3` by default) and, for Takeout, a preserved
...
- OAuth tokens (if you use `import-playlists`) -- stored at the path in
  `TUNE_HISTORY_GOOGLE_TOKEN_STORE` (default `./secrets/token.json`), read
...
- Browser cookies used for session-based watch-history import -- read
  ...
  by tune-history, never included in any export.
...
| Discogs (optional) | An `artist` + `track` (+ `album`) search query, plus your personal access token in the request | `identify` stage, only if `TUNE_HISTORY_DISCOGS_TOKEN` is set, once per unique query (cached after) | A second, independent source to corroborate or fill in a MusicBrainz miss |
...
`data/tune_history.sqlite3`, `data/raw_imports/`, and any exported CSV as
...
None. tune-history does not phone home, does not report usage analytics,
