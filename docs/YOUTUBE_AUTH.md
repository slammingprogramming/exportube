...
returns watch history through the official API. Exportube does not
...
## What Exportube does instead

`exportube import youtube` uses **your logged-in browser session** (via
...
- A real, currently logged-in browser session on the machine running
  exportube (`--cookies-from-browser chrome`/`firefox`/`edge`/...), or
...
2. Set `EXPORTUBE_GOOGLE_CLIENT_SECRETS_FILE` in `.env` to its path.
3. Run:

```bash
exportube import-playlists
```

The first run opens a browser for you to authorize; the resulting token is
cached at `EXPORTUBE_GOOGLE_TOKEN_STORE` (default `./secrets/token.json`)
...
Exportube's own code needs changes.
